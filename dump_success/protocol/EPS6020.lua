...
function Init() TMW, MSV, LSV, LSA, LMW = 1000, 44, 70, 20, 1200 end

Init()

function SetVA(V, A, S, C)
    local V1, V2 = (V or SV) / OVD * 1000, (A or SA) / OAD * 1000
    can.send(1, (0x029C00F0 + id * 256),
             encode.fromHex(DTH(V1, "%08X") .. DTH(V2, "%08X")))
    if S then
...
function SetPON(V)
    PON = (V or 0)
    can.send(1, (0x029A00F0 + id * 256),
             encode.fromHex(DTH(PON, "%02X") .. "00000000000000"))
...
function CANRecv(Format, ID, Data)
    ...
    if ID ~= (0x0286f000 + id) and ID ~= 0x0281f03f and ID ~= (0x0289f000 + id) and
        ID ~= (0x028AF000 + id) then return end
...
tmr.create():alarm(1000, 1, function()
    ...
    ShowMSG(MSG)
    can.send(1, (0x028900F0 + id * 256), X0)
end)
can.setup({
    speed = 125,
    tx = 5,
...
tmr.create():alarm(3000, 0,
                   function() can.send(1, (0x028A00F0 + id * 256), X0) end)
