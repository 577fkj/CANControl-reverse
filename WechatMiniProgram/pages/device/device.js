...
Page({
  ...
  Send: function () {
    ...
    if (3 == this.data.ShowPan)
      ...
      a.default.hex_md5("Check" + u + "@t*s$e#T.'1)`").match(/(........)$/),
        1 == Es && wx.setClipboardData({
          data: RegExp.$1
        });
    } else
      "重置设置" == u ?
      ...
      if ("系统设置" == u) Es >= 1 && this.setData({
        ...
        if (u.startsWith("0x")) af(u, 1, this);
          ...
          (u = "Setcfg('WFi',0,1)"), Qs("WIFI已设置为长期关闭模式\n重启生效");
              ...
              .exec(function (f) {
                ...
                for (
                  ...
                  for (
                    ...
                    (m =
                      m + "0x" + parseInt(Number(c), 2).toString(16) + ","),
                    (c = ""),
                    (m =
                      m +
                      "0x" +
                  ...
                  for (var h = 15; h > -1; h -= 1) {
                    ...
                    (x =
                      x + "0x" + parseInt(Number(c), 2).toString(16) + ","),
                    (c = ""),
                    (x =
                      x +
                      "0x" +
          ...
          if ("设置输出电流" == f)
            ...
            t >= 300 && t <= 800 && (u = 'SetCFG("MFs",' + t + ",1);SetFan(0)");
          else if ("充满延时关机" == f)
            t >= 0 && t < 1200 && (u = 'SetCFG("SD",' + t + ",1);SDT=SD*60");
          else if ("设置功率限制" == f)
            t > j || t < 100 ?
            Qs("超出设置范围") :
            (u = 'SetCFG("MW",' + t + ",1)");
            ...
            if ((console.log("Msg:" + s), s.match(/(<R><S>|<R><W>)/))) Xs(f);
              ...
              for (var Us = 0; Us < 8; Us++) {
                ...
                i.fillText(J[Us], 5, Us * Ls + 22),
                  ...
                  (i.textAlign = "right");
                var tf =
                  "string" == typeof bs[Us] ? Number(sf(s, bs[Us])) : bs[Us],
                  df =
                  "string" == typeof gs[Us] ? Number(sf(s, gs[Us])) : gs[Us];
                "Ti" == bs[Us] ?
              ...
              ((ms = sf(s, "KS1", "58.8V15A") + "A"),
                ...
                (Gs = sf(s, "TMS", "70C3000W")).match(/([\d\.]+)C([\d\.]+)W/),
                (Vs = Number(RegExp.$1)),
                (qs = Number(RegExp.$2)),
                ...
                ((X = (58.5 * K).toFixed(2)), (H = (41.5 * K).toFixed(2))) :
                f.data.Ver.includes("ZTE") ?
                ((X = (59 * K).toFixed(2)), (H = (41.5 * K).toFixed(2))) :
                ((X = R), (H = L)),
                (X = Number(X)),
                (H = Number(H)),
                (r[2] = vs),
...
function af(s, f, t) {
  ...
  if ((4 != (f = f.toLowerCase()).length && 8 != f.length) || !u.endsWith(f))
    wx.showToast({
      title: "设备认证失败\n" + s,
      icon: "error",
      duration: 2e3
