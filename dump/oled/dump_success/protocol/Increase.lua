...
function CANSend(C, V1, V2, V3)
    V1 = string.format("%04X", V1)
    V2 = string.format("%08X", V2)
    C = encode.fromHex(C .. V1 .. V2)
    can.send(1, CID + (V3 or DS), C)
end
...
function CANRecv(Format, ID, Data)
    ...
    if ID == 0x1207C081 and C == 1 then
        ...
        OW = OV * OA
        Set.OW = OW
    elseif ID == 0x12008081 then
        OT = HTD(T:sub(9, 12)) / 10
        Set0[i].OT = OT
    elseif ID == 0x1207A081 then
        IV = HTD(T:sub(5, 8)) / 32
        Set0[i].IV = IV
    end
end
